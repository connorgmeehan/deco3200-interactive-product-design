...
class EmotionDisplay {
    ...
    EmotionDisplay(DashedLines _dash) {
        ...
        rect1 = new DashedRectDrawer(dash, 825, 150, WHITE, "P1");
        rect2 = new DashedRectDrawer(dash, 1125, 150, WHITE, "P2");
        rect3 = new DashedRectDrawer(dash, 825, 350, WHITE, "P3");
        rect4 = new DashedRectDrawer(dash, 1125, 350, WHITE, "P4");
        rect5 = new DashedRectDrawer(dash, 825, 550, WHITE, "P11"); 
        rect6 = new DashedRectDrawer(dash, 1125, 550, WHITE, "P12");
        rect7 = new DashedRectDrawer(dash, 825, 750, WHITE, "P13"); 
        rect8 = new DashedRectDrawer(dash, 1125, 750, WHITE, "P14");
    ...
    void setup(String fakeId, String[] face) {
        // top left (0,0)
        String[] faces1Strings = getStringSegment(face, 0, 0.25, 0, 0.5); 
        faces1 = new TextDrawer(faces1Strings, 825, 150, 255, 6, font, 4);
        // bottom mid left (2,0) 
        String[] faces2Strings = getStringSegment(face, 0.25, 0.5, 0, 0.5); 
        faces2 = new TextDrawer(faces2Strings, 1125, 150, 255, 6, font, 4);
        // top right (0,1)
        String[] faces3Strings = getStringSegment(face, 0.25, 0.5, 0.5, 1); 
        faces3 = new TextDrawer(faces3Strings, 825, 350, 255, 6, font, 4); 
        //bottom mid right (2,1)
        String[] faces4Strings = getStringSegment(face, 0, 0.25, 0.5, 1);
        faces4 = new TextDrawer(faces4Strings, 1125, 350, 255, 6, font, 4);
        // top mid left (1,0)
        String[] faces5Strings = getStringSegment(face, 0.5, 0.75, 0.5, 1); 
        faces5 = new TextDrawer(faces5Strings, 825, 550, 255, 6, font, 4); 
        // bottom left (3,0)
        String[] faces6Strings = getStringSegment(face, 0.5, 0.75, 0, 0.5); 
        faces6 = new TextDrawer(faces6Strings, 1125, 550, 255, 6, font, 4);
        // top mid right (1,1)
        String[] faces7Strings = getStringSegment(face, 0.75, 1, 0, 0.5);
        faces7 = new TextDrawer(faces7Strings, 825, 750, 255, 6, font, 4); 
        // bottom right (3,1)
        String[] faces8Strings = getStringSegment(face, 0.75, 1, 0.5, 1);
    ...
    String[] getStringSegment(String[] _toSegment, float _iter, float _range, float _startSub, float _endSub) {    
        int startRow = int(_iter * float( _toSegment.length));
        int endRow = int(_range * float(_toSegment.length));

        ArrayList<String> returnValue = new ArrayList<String>();

        for (int i = startRow; i < endRow; i++) {
            int startCol = int(_startSub * float(_toSegment[0].length()));
            int endCol = int(_endSub * float(_toSegment[0].length()));
